...
From [**free Code Camp**](https://learn.freecodecamp.org/front-end-libraries/redux)

- Single State Object
- Is housed in the Redux Store (Update store must do it through the Redux store )
- createStore() : is a reducer method
- Redux Methods are available from a Redux Object and provides several of them.
---
...
- `js getState()` _retrieve_(recupera) the current state. **example**;
...
- Action is a simply js object. Sometimes also carries some data but is optional. It must carrying a TYPE property.
...
- Action creators sending action to the store. Is a simple js Function and returns an action. Is like a calling an action.
...
- DISPATCH: dispatch the action to Redux Store. call action creators and object with a type property. Then the method dispatches an action object to the Redux store.
...
// Dispatch the action here, the approach is that you can add a logic here and return the object that you need:
store.dispatch(loginAction);
//The same works for the next comment code
...
- Use If you need to call asynchronous endpoints. To include, pass it as an argument and a second optional parameter to the createStore(). like: 
 ```js
...
- NEVER MUTATE STATE: **You shouldn't mutate redux state.check this doc:** [Inmutable js](https://wecodetheweb.com/2016/02/12/immutable-javascript-using-es6-and-beyond/)
  **example Below**
```js
